...
public final class ZoneBorderRenderer {
    ...
    private static void drawWall(Matrix4f matrix, Vec3 cam, Vec3 look, double reach,
        ...
        // Ends even when nothing was written: the builder stays open otherwise and the next
        // frame inherits a half-built buffer.
        tesselator.end();
    ...
    private static boolean quad(BufferBuilder buf, Matrix4f m, Vec3 cam, Vec3 look,
        ...
        for (int corner = 0; corner < 4; corner++) {
            int at = corner * 5;
            buf.vertex(m, CORNERS[at], CORNERS[at + 1], CORNERS[at + 2])
                    .uv(CORNERS[at + 3] / TILE_BLOCKS + scroll,
                            CORNERS[at + 4] / TILE_BLOCKS - scroll)
                    .color((borderRgb >> 16) & 0xFF, (borderRgb >> 8) & 0xFF,
                            borderRgb & 0xFF, alpha).endVertex();
        }
        return true;
    }
