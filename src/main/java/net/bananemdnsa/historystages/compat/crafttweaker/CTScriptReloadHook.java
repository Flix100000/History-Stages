...
/**
 * The Forge-side wiring CraftTweaker needs: forward {@link StageEvent} to whatever the scripts
...
public final class CTScriptReloadHook {
    ...
    public static void register(IEventBus bus) {
        ...
        bus.addListener(net.minecraftforge.eventbus.api.EventPriority.NORMAL, false, StageEvent.Unlocked.class, event ->
                ScriptStageListeners.fireUnlocked(event.getStageId(), event.getDisplayName()));
        bus.addListener(net.minecraftforge.eventbus.api.EventPriority.NORMAL, false, StageEvent.Locked.class, event ->
                ScriptStageListeners.fireLocked(event.getStageId(), event.getDisplayName()));
        bus.addListener(net.minecraftforge.eventbus.api.EventPriority.NORMAL, false, StageEvent.IndividualUnlocked.class, event ->
                ScriptStageListeners.fireIndividualUnlocked(
                        event.getStageId(), event.getDisplayName(), event.getPlayerUUID()));
        bus.addListener(net.minecraftforge.eventbus.api.EventPriority.NORMAL, false, StageEvent.IndividualLocked.class, event ->
                ScriptStageListeners.fireIndividualLocked(
                        event.getStageId(), event.getDisplayName(), event.getPlayerUUID()));

        bus.addListener(net.minecraftforge.eventbus.api.EventPriority.NORMAL, false, AddReloadListenerEvent.class, event -> {
