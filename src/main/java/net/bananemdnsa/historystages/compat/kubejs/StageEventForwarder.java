...
public final class StageEventForwarder {
    ...
    public static void register(IEventBus bus) {
        bus.addListener(EventPriority.NORMAL, false, StageEvent.Unlocked.class, event ->
                HistoryStagesKubeEvents.UNLOCKED.post(
                        new HistoryStagesKubeEvents.StageKubeEvent(
                                event.getStageId(), event.getDisplayName()),
                        event.getStageId()));

        bus.addListener(EventPriority.NORMAL, false, StageEvent.Locked.class, event ->
                HistoryStagesKubeEvents.LOCKED.post(
                        new HistoryStagesKubeEvents.StageKubeEvent(
                                event.getStageId(), event.getDisplayName()),
                        event.getStageId()));

        bus.addListener(EventPriority.NORMAL, false, StageEvent.IndividualUnlocked.class, event ->
        ...
        bus.addListener(EventPriority.NORMAL, false, StageEvent.IndividualLocked.class, event ->
