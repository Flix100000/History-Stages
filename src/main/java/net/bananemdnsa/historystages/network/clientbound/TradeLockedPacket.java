...
public record TradeLockedPacket(int containerId, List<String> stageNames, TradeLockKind kind) {
    ...
    public static void encode(TradeLockedPacket msg, FriendlyByteBuf buf) {
    ...
    public static TradeLockedPacket decode(FriendlyByteBuf buf) {
    ...
    public static void handle(TradeLockedPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> TradeLockNotice.set(msg.containerId(), msg.stageNames(), msg.kind()));
